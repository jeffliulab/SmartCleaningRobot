...
ln -s /path/to/SmartCleaningRobot/deployment/cleaning_robot_ros ~/cleaning_ws/src/
cd ~/cleaning_ws
...
│   │   ├── simple_policy.py               ← 简单巡逻策略 (bump-and-turn)
│   │   ├── obs_builder.py                 ← ROS 消息 → 1066 维 obs 张量
...
```bash
python deployment/scripts/export_model.py \
    --checkpoint logs/skrl/cleaning_coverage/YYYY-MM-DD_HH-MM-SS/checkpoints/best_agent.pt \
    --output deployment/models/best_agent_scripted.pt
```
