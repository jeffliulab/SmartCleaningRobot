"""ROS 2 node that runs a cleaning or maze policy on live sensor data.
...
class PolicyNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("exit_y", -3.15)
        self.declare_parameter("maze_diagonal", 5.7)
        ...
        if self._policy_type in ("model", "maze_model"):
            import torch
        ...
        self._maze_obs = MazeObsBuilder(
            exit_x=self.get_parameter("exit_x").value,
            exit_y=self.get_parameter("exit_y").value,
            maze_diagonal=self.get_parameter("maze_diagonal").value,
            lidar_num_rays=self.get_parameter("lidar_num_rays").value,
            lidar_downsample_factor=self.get_parameter("lidar_downsample_factor").value,
            lidar_max_distance=self.get_parameter("lidar_max_distance").value,
        )
    ...
    def _on_scan(self, msg: LaserScan) -> None:
        ...
        self._maze_obs.update_lidar(msg.ranges)
    ...
    def _on_odom(self, msg: Odometry) -> None:
        ...
        self._maze_obs.update_pose(msg.pose.pose)
        self._maze_obs.update_velocity(msg.twist.twist)
    ...
    def _step(self) -> None:
        ...
        if self._policy_type == "maze_model" and self._model is not None:
            if not self._maze_obs.ready():
                return
            import torch
            obs_tensor = self._maze_obs.build()
            with torch.inference_mode():
                action = self._model(obs_tensor)
            cmd.linear.x = float(action[0, 0]) * self._max_v
            cmd.angular.z = float(action[0, 1]) * self._max_w

        elif self._policy_type == "model" and self._model is not None:
